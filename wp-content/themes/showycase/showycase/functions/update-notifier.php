 ...
 * Original code courtesy of Jo‹o Araœjo of Unisphere Design - http://themeforest.net/user/unisphere
 */
 
if(function_exists('simplexml_load_string')): //FOR SOME PHP VERSION ISSUES

function update_notifier_menu() {  
	$xml = get_latest_theme_version(3600); // This tells the function to cache the remote call for 3600 seconds (1 hour)
	...
	if(version_compare($theme_data['Version'], $xml->latest) == -1) {
		add_dashboard_page( $theme_data['Name'] . __('Theme Updates', 'premitheme'), $theme_data['Name'] . '<span class="update-plugins count-1"><span class="update-count">Update</span></span>', 'administrator', strtolower($theme_data['Name']) . '-updates', 'update_notifier' );
	}
}  
...
// UPDATE NOTIFICATION IN ADMIN BAR
function update_notifier_bar_menu() {
	...
	if(version_compare($theme_data['Version'], $xml->latest) == -1) {
		$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . $theme_data['Name'] . ' <span id="ab-updates">Update</span></span>', 'href' => get_admin_url() . 'index.php?page=' . strtolower($theme_data['Name']) . '-updates' ) );
	}
}
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://themes.premitheme.com/showycase/notifier.xml';
